...
void Animation::Add(int spriteId, DWORD time)
{
	...
	if (sprite == NULL)
	{
		DebugOut((LPWSTR)L"[ERROR] Sprite ID %d cannot be found!\n", spriteId);
	}
...
LPANIMATION Animations::Get(int id)
{
	...
	if (ani == NULL)
		DebugOut((LPWSTR)L"[ERROR] Failed to find animation id: %d\n", id);
	return ani;
}
...
LPANIMATION_SET AnimationSets::Get(unsigned int id)
{
	...
	if (ani_set == NULL)
		DebugOut((LPWSTR)L"[ERROR] Failed to find animation set id: %d\n", id);
